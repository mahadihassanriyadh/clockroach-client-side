    ...
    return (
        ...
        <Container>
            <Grid className="mt-3" container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                ...
                <Grid item xs={12} md={6}>
                    <p className="text-start mb-0"><span className="fw-bold">Size: </span>{orderItem.size}</p>
                    <p className="text-start mb-0"><span className="fw-bold">Product Code: </span>{orderItem.productCode}</p>
                    <p className="text-start mb-0"><span className="fw-bold">Category: </span>{orderItem.category}</p>
                    <p className="text-start mb-0"><span className="fw-bold">Product Material: </span>{orderItem.bodyMaterial} body</p>
                    <p className="text-start text-justify"><span className="fw-bold"></span>{orderItem.description}</p>
                    
                </Grid>
            </Grid>
            <div className="placeOrder">
                <h1 className="mt-3">Place an Order</h1>
                <h4 className="text-danger">Subtoal: {orderItem.price} BDT</h4>
                {
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <input className="form-control" type="text" {...register("district")} placeholder="District" required />
                    <textarea className="form-control" {...register("address")} placeholder="Address" required />
                    <input defaultValue={new Date().toLocaleString()} className="form-control" type="" {...register("time")} placeholder="Time" required readOnly/>
